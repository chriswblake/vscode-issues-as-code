...
suite("Extension – integration tests", () => {
  ...
  test("issuesAsCode.pullNow command is registered", async () => {
    ...
    assert.ok(
      commands.includes("issuesAsCode.pullNow"),
      "issuesAsCode.pullNow should be registered",
    );
  });
  ...
  test("issuesAsCode.pushNow command is registered", async () => {
    const commands = await vscode.commands.getCommands(true);
    assert.ok(
      commands.includes("issuesAsCode.pushNow"),
      "issuesAsCode.pushNow should be registered",
    );
  });
  ...
  test("issuesAsCode.addOpenIssuesDefaultConfig command is registered", async () => {
    const commands = await vscode.commands.getCommands(true);
    assert.ok(
      commands.includes("issuesAsCode.addOpenIssuesDefaultConfig"),
      "issuesAsCode.addOpenIssuesDefaultConfig should be registered",
    );
  });
});
