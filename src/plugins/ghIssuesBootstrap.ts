...
export function registerGhIssuesCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "issuesAsCode.addOpenIssuesDefaultConfig",
      async () => {
        ...
        if (
          hasDuplicateGhIssuesTarget(
            currentTargets,
            (f) => f?.repository === repository && f?.state === "open",
          )
        ) {
          void vscode.window.showInformationMessage(
            `Open issues sync target already exists for ${repository}.`,
          );
          return;
        }
        ...
        await reinitialize();
        void vscode.window.showInformationMessage(
          `Added default open issues sync target for ${repository}.`,
        );
      },
    ),

    vscode.commands.registerCommand(
      "issuesAsCode.addMyIssuesOnGitHub",
      async () => {
        ...
        if (!username) {
          void vscode.window.showErrorMessage(
            "Could not authenticate with GitHub. Please sign in.",
          );
          return;
        }
        ...
        if (
          ...
          void vscode.window.showInformationMessage(
            `"My open issues on GitHub" sync target already exists for ${username}.`,
          );
          return;
        }
        ...
        await reinitialize();
        void vscode.window.showInformationMessage(
          `Added "My open issues on GitHub" sync target for ${username}.`,
        );
      },
    ),

    vscode.commands.registerCommand(
      "issuesAsCode.addMyIssuesOnThisRepo",
      async () => {
        const { folder, repoInfo } = await requireWorkspaceRepo();
        if (!folder || !repoInfo) {
          return;
        ...
        if (
          ...
          void vscode.window.showInformationMessage(
            `"My open issues on this repository" sync target already exists for ${username} on ${repository}.`,
          );
          return;
        }
        ...
        await reinitialize();
        void vscode.window.showInformationMessage(
          `Added "My open issues on this repository" sync target for ${username} on ${repository}.`,
        );
      },
    ),
  );
...
export const bootstrap: PluginBootstrap = {
  ...
  registerCommands(
    ...
    registerGhIssuesCommands(
      context as vscodeType.ExtensionContext,
      reinitialize,
    );
  },
