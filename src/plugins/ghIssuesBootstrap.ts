 ...
 */
export function hasDuplicateGhIssuesTarget(currentTargets: SyncTarget[], predicate: (filters: Record<string, unknown> | undefined) => boolean): boolean {
 ...
 */
export function registerGhIssuesCommands(context: vscodeType.ExtensionContext, reinitialize: () => Promise<void>): void {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const vscode = require('vscode') as typeof vscodeType;

  context.subscriptions.push(
    vscode.commands.registerCommand('issuesAsCode.addSyncTargetRepoIssues', async () => {
      ...
      if (hasDuplicateGhIssuesTarget(currentTargets, (f) => f?.repository === repository && f?.state === 'open' && !f?.assignee)) {
      ...
      await reinitialize();
      void vscode.window.showInformationMessage(`Added open issues sync target for ${repository}.`);
    }),

    vscode.commands.registerCommand('issuesAsCode.addSyncTargetMyIssuesOnGitHub', async () => {
      ...
      const cfg = vscode.workspace.getConfiguration('issuesAsCode', folder.uri);
      const currentTargets = cfg.get<SyncTarget[]>('syncTargets') ?? [];
      if (hasDuplicateGhIssuesTarget(currentTargets, (f) => f?.assignee === username && f?.state === 'open' && !f?.repository)) {
        void vscode.window.showInformationMessage(`"My open issues on GitHub" sync target already exists for ${username}.`);
        return;
      }

      await cfg.update('syncTargets', [...currentTargets, target], vscode.ConfigurationTarget.WorkspaceFolder);
      await reinitialize();
      void vscode.window.showInformationMessage(`Added "My open issues on GitHub" sync target for ${username}.`);
    }),

    vscode.commands.registerCommand('issuesAsCode.addSyncTargetAssignedRepoIssues', async () => {
      const { folder, repoInfo } = await requireWorkspaceRepo();
      if (!folder || !repoInfo) {
        return;
      }
      ...
      if (hasDuplicateGhIssuesTarget(currentTargets, (f) => f?.repository === repository && f?.assignee === username)) {
        void vscode.window.showInformationMessage(`"My open issues on this repository" sync target already exists for ${username} on ${repository}.`);
        return;
      }

      await cfg.update('syncTargets', [...currentTargets, target], vscode.ConfigurationTarget.WorkspaceFolder);
      await reinitialize();
      void vscode.window.showInformationMessage(`Added "My open issues on this repository" sync target for ${username} on ${repository}.`);
    }),
  );
}
...
async function requireWorkspaceRepo(): Promise<{
  folder?: vscodeType.WorkspaceFolder;
  repoInfo?: RepoInfo;
}> {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const vscode = require('vscode') as typeof vscodeType;
...
export const bootstrap: PluginBootstrap = {
  ...
  registerCommands(context: { subscriptions: { dispose(): void }[] }, reinitialize: () => Promise<void>): void {
  ...
  async persistDefaults(workspaceFolder: { uri: { fsPath: string } }): Promise<SyncTarget[] | null> {
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const vscode = require('vscode') as typeof vscodeType;
      const folder = vscode.workspace.workspaceFolders?.find((f: vscodeType.WorkspaceFolder) => f.uri.fsPath === workspaceFolder.uri.fsPath);
      if (!folder) {
        return null;
      }
      ...
      const target = openIssuesTarget(repoInfo.owner, repoInfo.repo);
      await cfg.update('syncTargets', [target], vscode.ConfigurationTarget.WorkspaceFolder);
