...
suite('Extension – integration tests', () => {
  ...
  test('issuesAsCode.addSyncTargetRepoIssues command is registered', async () => {
    const commands = await vscode.commands.getCommands(true);
    assert.ok(commands.includes('issuesAsCode.addSyncTargetRepoIssues'), 'issuesAsCode.addSyncTargetRepoIssues should be registered');
  });
});
